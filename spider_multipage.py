...
artist_page_num=0
while(True):
    artist_page_num=artist_page_num+1
    artists_list=[]
    artists_name_list=[]
...
# urllib2.install_opener(opener)
            ...
            for index,songs_url in enumerate(songs_list):
                ...
                file_object.close( )
            if(len(soup.findAll('a',{'class':"p_redirect_l"}))==0 or song_page_num==5):
                break
            else:
                singer_url=ori_url+((soup.findAll('a',{'class':"p_redirect_l"}))[0])['href']
    if(len(soup.findAll('a',{'class':"p_redirect_l"}))==0 or artist_page_num==8):
        break
    else:
